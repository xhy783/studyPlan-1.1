        ...
        fs.createReadStream(filePath)
            ...
            .on('end', () => {
                results.forEach(row => {
                    ...
                    if (['中班', '小班', '大班', '幼小衔接', '初一', '初二', '初三', '高一', '高二', '高三', '1年级', '2年级' ,'3年级' ,'4年级' ,'5年级' ,'6年级' ].includes(grade)) {
        ...
        output.on('close', () => {
            res.download(outputZip, err => {
                if (err) {
                    ...
                    fs.rm(processedFolder, { recursive: true, force: true }, (err) => {
                        if (err) {
                            console.error(`Error removing folder ${processedFolder}:`, err);
                        }
                    });
